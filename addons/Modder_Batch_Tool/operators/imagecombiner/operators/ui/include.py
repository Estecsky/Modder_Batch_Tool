...
def _materials_list(scn: Scene, m_col: bpy.types.UILayout) -> None:
    ...
    col = m_col.column()
    col.scale_y = 1.5
    col.operator('smc.combiner', text='Save Atlas to..').cats = True
    col.separator()
    col = m_col.column()
    col.label(text='If this saved you time:')
    col.operator('smc.browser', text='Support Material Combiner').link = patreon
    col.operator('smc.browser', text='Buy Me a Coffee').link = buymeacoffee
